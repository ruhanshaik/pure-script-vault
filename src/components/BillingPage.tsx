...
function BillViewModal({ bill, onClose }: { bill: Bill | null; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-foreground/30 backdrop-blur-sm p-4">
      <div className="w-full max-w-lg rounded-2xl bg-card border border-border shadow-2xl p-4 sm:p-6 max-h-[90vh] overflow-y-auto">
...
function NewBillPanel({ onCreated }: { onCreated: () => void }) {
  ...
  useEffect(() => { getMedicines().then(setMedicines); }, []);
  ...
  const addItem = (med: Medicine) => {
    ...
    setItems(prev => [...prev, { medicineId: med.id, medicineName: med.name, batchNumber: med.batchNumber, quantity: 1, mrp: med.mrp, discount: 0, gstRate: med.gstRate, total: med.mrp }]);
    setSearchMed('');
  };
  ...
  const handleSubmit = async () => {
    ...
    await addBill(bill);
    setSaving(false);
    setItems([]); setPatientName(''); setDoctorName('');
    onCreated();
  };
      ...
      {items.length > 0 && (
        <div className="flex flex-col sm:flex-row justify-between items-end gap-4">
          ...
          <div className="w-full sm:w-72 space-y-1.5 text-sm">
            ...
            <button onClick={handleSubmit} disabled={saving} className="btn-primary w-full !mt-3">{saving ? 'Creating...' : 'Create Bill'}</button>
          </div>
        </div>
      )}
    </div>
...
export function BillingPage() {
  ...
  const loadBills = () => { getBills().then(b => { setBills(b); setIsLoading(false); }); };
  useEffect(() => { loadBills(); }, []);
  ...
  return (
    <div className="space-y-4 sm:space-y-5">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          <h1 className="text-lg sm:text-xl font-bold text-foreground">Billing & POS</h1>
          <p className="text-xs sm:text-sm text-muted-foreground">{bills.length} total bills</p>
        </div>
        <button onClick={() => setShowNew(!showNew)} className="btn-primary self-start sm:self-auto">
          <IconPlus className="h-4 w-4" /> {showNew ? 'Hide New Bill' : 'New Bill'}
        </button>
      </div>
      {showNew && <NewBillPanel onCreated={loadBills} />}
      <div className="rounded-xl border border-border bg-card overflow-hidden">
        <div className="px-4 sm:px-5 py-3 border-b border-border"><h3 className="text-sm font-semibold text-foreground">Bill History</h3></div>
        <div className="overflow-x-auto">
          <table className="w-full text-sm min-w-[600px]">
            <thead><tr className="border-b border-border bg-muted/40">
              <th className="px-3 sm:px-4 py-3 text-left text-xs font-semibold text-muted-foreground">Bill No</th>
              <th className="px-3 sm:px-4 py-3 text-left text-xs font-semibold text-muted-foreground">Patient</th>
              <th className="px-3 sm:px-4 py-3 text-right text-xs font-semibold text-muted-foreground">Amount</th>
              <th className="px-3 sm:px-4 py-3 text-center text-xs font-semibold text-muted-foreground">Payment</th>
            ...
            <tbody>
              {bills.length === 0 ? (
                <tr><td colSpan={7}><EmptyState message="No bills yet" /></td></tr>
              ) : bills.map(b => (
                <tr key={b.id} className="border-b border-border last:border-0 table-row-hover">
                  <td className="px-3 sm:px-4 py-3 font-medium text-foreground">{b.billNumber}</td>
                  <td className="px-3 sm:px-4 py-3 text-muted-foreground">{b.patientName}</td>
                  <td className="px-3 sm:px-4 py-3 text-right font-medium">Rs. {b.grandTotal.toFixed(2)}</td>
                  <td className="px-3 sm:px-4 py-3 text-center"><span className="badge badge-info">{b.paymentMethod}</span></td>
                  <td className="px-3 sm:px-4 py-3 text-center"><span className={`badge ${b.status === 'Completed' ? 'badge-success' : b.status === 'Pending' ? 'badge-warning' : 'badge-danger'}`}>{b.status}</span></td>
                  <td className="px-3 sm:px-4 py-3 text-muted-foreground text-xs">{new Date(b.createdAt).toLocaleDateString()}</td>
                  <td className="px-3 sm:px-4 py-3"><button onClick={() => setViewBill(b)} className="text-primary hover:opacity-70"><IconEye className="h-4 w-4" /></button></td>
                </tr>
              ))}
            </tbody>
          </table>
