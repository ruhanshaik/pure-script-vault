...
export function AccountingPage() {
  const { data: bills = [], isLoading: bLoading } = useQuery({ queryKey: ['bills'], queryFn: getBills });
  ...
  if (isLoading) return <div className="grid grid-cols-4 gap-5">{Array.from({ length: 4 }).map((_, i) => <SkeletonCard key={i} />)}</div>;

  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-xl font-bold text-foreground">Financial Accounting</h1>
        <p className="text-sm text-muted-foreground">Overview of revenue, expenses, and taxes</p>
      </div>
      ...
      <div className="grid grid-cols-4 gap-5">
        {[
          ...
          <div key={item.label} className="kpi-card">
            ...
            <p className={`text-2xl font-bold mt-2 ${item.color}`}>Rs. {item.value.toLocaleString('en-IN', { maximumFractionDigits: 0 })}</p>
          </div>
        ))}
      </div>
