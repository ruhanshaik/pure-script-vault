...
export function AdminPage() {
  const { data: logs = [], isLoading } = useQuery({ queryKey: ['activityLogs'], queryFn: getActivityLogs });
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">User management, roles, and activity logs</p>
      </div>

      {/* Users */}
      <div className="rounded-xl border border-border bg-card overflow-hidden">
        <div className="px-5 py-3 border-b border-border"><h3 className="text-sm font-semibold text-foreground">Users & Roles</h3></div>
        <table className="w-full text-sm">
          <thead><tr className="border-b border-border bg-muted/40">
            ...
            <th className="px-4 py-3 text-center text-xs font-semibold text-muted-foreground">Role</th>
          ...
          <tbody>{mockUsers.map(u => (
            <tr key={u.id} className="border-b border-border last:border-0 table-row-hover">
              ...
              <td className="px-4 py-3 text-center"><span className={`badge ${u.role === 'Admin' ? 'badge-danger' : u.role === 'Pharmacist' ? 'badge-info' : 'badge-warning'}`}>{u.role}</span></td>
              <td className="px-4 py-3 text-center"><span className={`badge ${u.isActive ? 'badge-success' : 'badge-danger'}`}>{u.isActive ? 'Active' : 'Inactive'}</span></td>
            </tr>
          ))}</tbody>
        </table>
      </div>
      ...
      {/* Activity Logs */}
      <div className="rounded-xl border border-border bg-card overflow-hidden">
        <div className="px-5 py-3 border-b border-border"><h3 className="text-sm font-semibold text-foreground">Activity Logs</h3></div>
        <table className="w-full text-sm">
          <thead><tr className="border-b border-border bg-muted/40">
          ...
          <tbody>{logs.map(l => (
            <tr key={l.id} className="border-b border-border last:border-0 table-row-hover">
              ...
              <td className="px-4 py-3"><span className="badge badge-info">{l.module}</span></td>
